...
for f in xrange(len(fname)):
    if not(p==f) and not(p2==f):
        perf[f] = perf[f]/((params[f]['n_blocks']-start)*(back-to-1.))
    elif not(p2==f):
        perf[p] = perf[p]/((params[p]['n_blocks']-startpd)*(back-to-1.))
    else:
        perf[p2] = perf[p2]/((params[p2]['n_blocks']-startpd)*(back-to-1.))
...
ax.annotate("", xy=(5.4, .47), xycoords='data', 
    xytext=(1.6, .47), textcoords='data',
...
ax.set_ylabel('Average success ratio for\nthe first '+str(int(back-to))+' trials of each block', multialignment='center')

pl.savefig('anovatest_start.pdf', bbox_inches='tight', dpi=600)
pl.savefig('anovatest_start.eps', bbox_inches='tight', dpi=600)
pl.savefig('anovatest_start.tiff', bbox_inches='tight', dpi=600)
pl.savefig('anovatest_start.png', bbox_inches='tight', dpi=600)
